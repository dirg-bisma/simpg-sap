...
GROUP BY a.`kode_blok`")->result();
			...
			foreach ($sqldo as $k) {
				...
				$dt = array(
					...
					'total_pendapatan'=>($k->gula_90*$period->harga_gula)+($k->tetes_ptr*$period->harga_tetes),
					'total_potongan'=>0,
					'total_pendapatan_bersih'=>($k->gula_90*$period->harga_gula)+($k->tetes_ptr*$period->harga_tetes),
