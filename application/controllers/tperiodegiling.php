...
GROUP BY a.`kode_blok`")->result();
			...
			foreach ($sqldo as $k) {
				...
				$dt = array(
					...
					'total_pendapatan'=>round(($gulado*$period->harga_gula))+round(($k->tetes_ptr*$period->harga_tetes)),
					'total_potongan'=>0,
					'total_pendapatan_bersih'=>round(($gulado*$period->harga_gula))+round(($k->tetes_ptr*$period->harga_tetes)),
