...
<section class="content">
      ...
      switch ($gid) {
        ...
        case '10':
          ?>
              <li><a href="<?php echo site_url('tsbh/upload');?>"><i class="fa fa-upload"></i> Upload SBH  </a></li> 
              <li class="active"><a href="<?php echo site_url('tsbh/pengolahan');?>"><i class="fa fa-check-square"></i> Approve Pengolahan </a></li> 
          <?php
          break;
        case '11':
          ?>
    ...
    ?>
              ...
              <div class="box box-danger">
                  ...
                  <h3 class="box-title">Approve Pengolahan</h3>
		...
		<a href="javascript:approved()" class="tips btn btn-xs btn-success"  title="View">
...
table = $('#gridv').DataTable({
          ...
          "ajax": {
            "url": "<?php echo site_url('tsbh/grids')?>/2/"+$('#tgl1').val()+'/'+$('#tgl2').val(),
            "type": "POST"
          }, 
...
function reloadGrid(){
       // $("#gridv").DataTable();
	   //table.destroy();
	   table.ajax.url( "<?php echo site_url('tsbh/grids')?>/2/"+$('#tgl1').val()+'/'+$('#tgl2').val() ).load();
 ...
 function approved(){
    ...
    type: "POST",
    data : {stt:1,cancel:0,tgl1:$('#tgl1').val(),tgl2:$('#tgl2').val()},
    beforeSend: function() {
 ...
 function cancelapproved(){
  $.ajax({
    url: "<?php echo site_url('tsbh/approved');?>",
    type: "POST",
    data : {stt:1,cancel:1,tgl1:$('#tgl1').val(),tgl2:$('#tgl2').val()},
    beforeSend: function() {
      return confirm("Apakah anda yakin approve data ini ?");
    },
    success: function(data){
