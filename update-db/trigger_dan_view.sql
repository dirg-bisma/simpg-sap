...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vw_ari_data` AS (select `a`.`no_spat` AS `no_spat`,`b`.`id_ari` AS `id_ari`,`b`.`id_spta` AS `id_spta`,`b`.`persen_brix_ari` AS `persen_brix_ari`,`b`.`persen_pol_ari` AS `persen_pol_ari`,`b`.`ph_ari` AS `ph_ari`,`b`.`hk` AS `hk`,`b`.`nilai_nira` AS `nilai_nira`,`b`.`faktor_rendemen` AS `faktor_rendemen`,`b`.`rendemen_ari` AS `rendemen_ari`,`b`.`hablur_ari` AS `hablur_ari`,`b`.`ditolak_ari` AS `ditolak_ari`,`b`.`ditolak_alasan` AS `ditolak_alasan`,`b`.`tgl_ari` AS `tgl_ari`,`b`.`ptgs_ari` AS `ptgs_ari` from (`t_spta` `a` join `t_ari` `b` on((`a`.`id` = `b`.`id_spta`)))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vw_sbh_data` AS (select sql_no_cache `a`.`sbh_status` AS `sbh_status`,`f`.`id_ari` AS `id_ari`,`a`.`id` AS `id`,`a`.`no_spat` AS `no_spat`,`a`.`kode_kat_lahan` AS `kode_kat_lahan`,`a`.`kode_plant` AS `kode_plant`,`a`.`kode_affd` AS `kode_affd`,`a`.`kode_blok` AS `kode_blok`,`a`.`tgl_spta` AS `tgl_spta`,`a`.`tebang_pg` AS `tebang_pg`,`a`.`angkut_pg` AS `angkut_pg`,`a`.`jenis_spta` AS `jenis_spta`,`c`.`no_angkutan` AS `no_angkutan`,`g`.`nama_petani` AS `nama_petani`,`b`.`deskripsi_blok` AS `deskripsi_blok`,`b`.`luas_ha` AS `luas_ha`,`c`.`ha_tertebang` AS `ha_tertebang`,`c`.`tgl_tebang` AS `tgl_tebang`,`c`.`brix_sel` AS `brix_sel`,`c`.`ph_sel` AS `ph_sel`,`c`.`ditolak_sel` AS `ditolak_sel`,`c`.`ditolak_alasan` AS `ditolak_alasan`,`a`.`cetak_spta_tgl` AS `cetak_spta_tgl`,`a`.`selektor_tgl` AS `selektor_tgl`,`a`.`timb_netto_tgl` AS `timb_netto_tgl`,`a`.`meja_tebu_tgl` AS `meja_tebu_tgl`,`a`.`ari_tgl` AS `ari_tgl`,`a`.`sbh_tgl` AS `sbh_tgl`,`a`.`hari_giling` AS `hari_giling`,`a`.`tgl_giling` AS `tgl_giling`,`d`.`bruto` AS `bruto`,`d`.`tara` AS `tara`,`d`.`netto_final` AS `netto_final`,`e`.`kondisi_tebu` AS `kondisi_tebu`,`e`.`daduk` AS `daduk`,`e`.`sogolan` AS `sogolan`,`e`.`pucuk` AS `pucuk`,`e`.`akar_tanah` AS `akar_tanah`,`e`.`non_tebu` AS `non_tebu`,`e`.`terbakar` AS `terbakar`,`e`.`cacahan` AS `cacahan`,`e`.`brondolan` AS `brondolan`,`f`.`persen_brix_ari` AS `persen_brix_ari`,`f`.`persen_pol_ari` AS `persen_pol_ari`,`f`.`ph_ari` AS `ph_ari`,`f`.`hk` AS `hk`,`f`.`nilai_nira` AS `nilai_nira`,`f`.`faktor_rendemen` AS `faktor_rendemen`,`f`.`rendemen_ari` AS `rendemen_ari`,round(if((`f`.`hablur_ari` = 0),((`f`.`rendemen_ari` * `d`.`netto_final`) / 100),`f`.`hablur_ari`),2) AS `hablur_ari`,round(if((`f`.`gula_total` = 0),(if((`f`.`hablur_ari` = 0),round(if((`f`.`hablur_ari` = 0),((`f`.`rendemen_ari` * `d`.`netto_final`) / 100),`f`.`hablur_ari`),2),`f`.`hablur_ari`) * 1.003),`f`.`gula_total`),2) AS `gula_total`,if((`f`.`tetes_total` = 0),round(((4.5 * `d`.`netto_final`) / 100),2),`f`.`tetes_total`) AS `tetes_total`,if((`f`.`rendemen_ptr` = 0),`get_rendemen_bagihasil_ptr`(left(`a`.`kode_kat_lahan`,2),`f`.`rendemen_ari`),`f`.`rendemen_ptr`) AS `rendemen_ptr`,round(if((`f`.`gula_ptr` = 0),(((`d`.`netto_final` * if((`f`.`rendemen_ptr` = 0),`get_rendemen_bagihasil_ptr`(left(`a`.`kode_kat_lahan`,2),`f`.`rendemen_ari`),`f`.`rendemen_ptr`)) / 100) * 1.003),`f`.`gula_ptr`),2) AS `gula_ptr`,round(if((`f`.`tetes_ptr` = 0),((3 * `d`.`netto_final`) / 100),`f`.`tetes_ptr`),2) AS `tetes_ptr`,round(if((`f`.`gula_pg` = 0),(round(if((`f`.`gula_total` = 0),(if((`f`.`hablur_ari` = 0),round(if((`f`.`hablur_ari` = 0),((`f`.`rendemen_ari` * `d`.`netto_final`) / 100),`f`.`hablur_ari`),2),`f`.`hablur_ari`) * 1.003),`f`.`gula_total`),2) - round(if((`f`.`gula_ptr` = 0),(((`d`.`netto_final` * if((`f`.`rendemen_ptr` = 0),`get_rendemen_bagihasil_ptr`(left(`a`.`kode_kat_lahan`,2),`f`.`rendemen_ari`),`f`.`rendemen_ptr`)) / 100) * 1.003),`f`.`gula_ptr`),2)),`f`.`gula_pg`),2) AS `gula_pg`,round(if((`f`.`tetes_pg` = 0),(if((`f`.`tetes_total` = 0),((4.5 * `d`.`netto_final`) / 100),`f`.`tetes_total`) - if((`f`.`tetes_ptr` = 0),((3 * `d`.`netto_final`) / 100),`f`.`tetes_ptr`)),`f`.`tetes_pg`),2) AS `tetes_pg` from ((((((`t_spta` `a` join `sap_field` `b` on((`a`.`kode_blok` = `b`.`kode_blok`))) join `t_selektor` `c` on((`c`.`id_spta` = `a`.`id`))) join `t_timbangan` `d` on((`d`.`id_spat` = `a`.`id`))) join `t_meja_tebu` `e` on((`e`.`id_spta` = `a`.`id`))) join `t_ari` `f` on((`f`.`id_spta` = `a`.`id`))) left join `sap_petani` `g` on((`g`.`id_petani_sap` = `b`.`id_petani_sap`))) group by `a`.`id`) */;
